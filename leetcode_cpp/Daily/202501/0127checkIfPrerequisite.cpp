  ...
  public:
    vector<bool> checkIfPrerequisite(int numCourses, vector<vector<int>>& prerequisites, vector<vector<int>>& queries) {
        ...
        for (auto& query : queries) {
            ...
            answer.push_back(evaluate_prereq_query(course1, course2, memo, prereq_for));
        }
        return answer;
    }

  private:
    const inline static auto not_evaluated = bitset<101>(1) << 100;
    ...
    bitset<101>& prereq_for_set(int course, vector<bitset<101>>& memo, const vector<vector<int>>& prereq_for) {
        auto& prereq_set = memo[course];
        if (prereq_set != not_evaluated) {
            return prereq_set;
        }
