...
class Solution {
  public:
    static vector<vector<int>> modifiedGraphEdges(int n, vector<vector<int>>& edges, int source, int destination,
                                                  int target) {
        ...
        while (!priorityQueue.empty()) {
            ...
            for (auto& neighbor : adjacencyList[currentNode]) {
                ...
                if (distances[nextNode][run] > distances[currentNode][run] + weight) {
                    ...
                    priorityQueue.emplace(distances[nextNode][run], nextNode);
                }
            }
        }
    }
